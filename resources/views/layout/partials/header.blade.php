                    ...
                    @if (Auth::user()->isArtist)
                        <form method="GET" action="{{ url('jobs-search') }}">
                            <div class="input-group hide-search-mobile" style="margin-bottom: 14px;">
                                ...
                                <input class="form-control " id="search_job" autocomplete="off"  name="search" type="text"
                                    placeholder="Search Here...."
                                    style="padding-bottom: 6px; width:846px;border: #cfcfcf 2px solid; border-radius:11px 0px 0px 11px;">
                                <button class="btn btn-warning" type="submit"
                                    style="background: #F79533;border-radius: 0px 11px 11px 0px;">
                                    <span class="text-capitalize"
                                        style="color: rgb(255,255,255);font-size: 14px;">Search</span></button>
                                <!-- </div> -->
                            </div>
                        </form>
                    ...
                    @if (Auth::user()->isArtist)
                        <ul class="main-nav hide-desktop">
                            <li class="{{ Request::is('index') ? 'active' : '' }}">
                                <a href="{{ url('artist') }}"><i class="material-icons">verified_user</i> Dashboard</a>

                            </li>

                            <li>
                                @if (Auth::user()->profile()->first()->id != null)
                                    <a href="{{ route('profshow',Auth::user()->profile()->first()->id) }}"><i
                                            class="material-icons">verified_user</i> View profile</a>
                                @endif

                            </li>
                            <li>
                                <a href="{{ url('project_list') }}"><i class="material-icons">business_center</i>
                                    Projects</a>

                            </li>
                            <li>
                                <a href="{{ url('albums') }}"><i class="material-icons">collections</i> Albums</a>

                            </li>
                            <li>
                                <a href="{{ url('payment') }}"> <i class="fas fa-dollar-sign"></i>Wallet <span
                                        class="amt"></span></a>

                            </li>
                            <li>
                                <a href="{{ url('basic') }}"> <i class="material-icons">settings</i> Profile
                                    Settings</a>
                ...
                <ul class="nav header-navbar-rht">
                    @if (!Route::is([
                        ...
                        @guest
                            ...
                            <li><a class="reg-btn" data-bs-toggle="modal" href="#login"><i class="fas fa-lock"></i>
                                    Login</a></li>
                        @else
                            ...
                            <li class="nav-item dropdown account-item">
                                ...
                                @if (Auth::user() != null)                                    
                                        <a href="{{ url('markasread') }}"  class="dropdown-toggle nav-link"
                                            data-bs-toggle="dropdown">                                       
                                            <i class="far fa-bell"></i>
                                            @if (auth()->user()->unreadNotifications->count() != 0)
                                                <span class="badge badge-pill" style="font-size: 8px !important;background:red;">
                                                    {{ auth()->user()->unreadNotifications->count() }}</span>
                                            @endif
                                        </a>
                                        <div class="dropdown-menu emp">                                     
                                                @foreach (Auth::user()->unreadNotifications()->limit(5)->get() as $notification)
                                                <p class="dropdown-item" style="padding: 0px;"><div class="avatar avatar-xs">
                                                <img class="avatar-img rounded-circle"  src="https://www.lytaa.com/public/assets/img/avatar-artist.jpg">
                                                </div>{{ $notification->data['data'] }}</p>
                                                    <br>
                                                @endforeach                                      
                                            <div class="drop-head"><a href="{{ url('artistnotification') }}">View All </a></div>
                                            @if(Auth::user()->unreadNotifications() == NULL)
                                                <span class="drop-head"><a href="{{ url('artistnotification') }}">View All </a></span>
                                                <span class="drop-head"><a href="{{ url('markasread') }}">Clear All </a></span>
                                            @endif
                                        </div>
                                ...
                                <a href="#" class="dropdown-toggle nav-link" data-bs-toggle="dropdown">
                                    @if(Auth::user()->profile()->first()->profile_img == null)									
                                    <span class="user-img">
										<img alt="" src="{{asset('assets/img/avatar-artist.jpg')}}" class="avatar-lg rounded-circle">
                                    </span>
                                    @else
                                    <span class="user-img">
                                        <img src="{{ asset('profile_img/'.Auth::user()->profile()->first()->profile_img) }}"
                                            alt="">
                                    </span>
                                    @endif                                 
                                </a>
                                <div class="dropdown-menu emp">
                                    ...
                                    @if (Auth::user()->isArtist)
                                        <a class="dropdown-item" href="{{ url('artist') }}"><i
                                                class="material-icons">verified_user</i> Dashboard</a>
                                    @else
                                    ...
                                    @if (Auth::user()->isArtist)
                                        @if (Auth::user()->profile()->first()->id != null)
                                            <a class="dropdown-item"
                                                href="{{ route('profshow',Auth::user()->profile()->first()->id) }}"><i
                                                    class="material-icons">verified_user</i> View profile</a>
                                        @endif
                                    @endif
                                    ...
                                    <a class="dropdown-item" href="{{ url('project_list') }}"><i
								...
								@endif --}}
                                    ...
                                    @if (Auth::user()->isArtist)
                                        <a class="dropdown-item" href="{{ url('albums') }}"><i
                                                class="material-icons">collections</i> Albums</a>
                                        <div class="drop-head">Financial Settings</div>
                                        <a class="dropdown-item bal" href="{{ url('payment') }}"> <i
                                                class="fas fa-dollar-sign"></i>Wallet <span class="amt"></span></a>
                                    @endif

                                    <div class="drop-head">Account Details</div>
                                    <a class="dropdown-item" href="{{ url('basic') }}"> <i
                    ...
                    @if (Route::is([
                        ...
                        <li class="nav-item dropdown has-arrow main-drop account-item">
                            ...
                            <div class="dropdown-menu emp">
                                <div class="drop-head">Account Details</div>
                                <a class="dropdown-item" href="user-account-details"><i
                                        class="material-icons">verified_user</i> View profile</a>
                                <div class="drop-head">Projects Settings</div>
                                <a class="dropdown-item" href="manage-projects"><i
                                        class="material-icons">business_center</i> Projects</a>
                                <a class="dropdown-item" href="favourites"><i class="material-icons">local_play</i>
                                    Favourites</a>
                                <a class="dropdown-item" href="review"><i class="material-icons">pie_chart</i>
                                    Reviews</a>
                                <div class="drop-head">Account Details</div>
                                <a class="dropdown-item" href="profile-settings"> <i
                                        class="material-icons">settings</i> Profile Settings</a>
                            ...
                            <div class="col-md-12">
                                <a href="{{ route('facebook') }}"><button type="button"
                                        class="icon-button social-signing-button facebook-signing-button"><span
                                            ...
                                            style="width: 18px; height: 18px;"><svg width="20" height="20"
                            ...
                            <div class="col-md-12">
                                <a href="{{ route('google') }}"><button type="button"
                                        class="icon-button social-signing-button google-signing-button"><span
                                            class="lFICM06 provider-icon" aria-hidden="true"
                                            style="width: 18px; height: 18px;"><svg width="18" height="19"
                                ...
                                <form method="POST" action="{{ route('login') }}" style="padding:28px;">
                                   ...
                                   <div class="form-check">
                                    ...
                                    <div class="text-center">
                                        <button class="btn btn-primary btn-block-login btn-sm login-btn"
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6 ">New to LYTAA? <a href="{{ route('beforesignup') }}">Click here</a>
                                        </div>
                                    </div>
                                </form>
                            </div>
