									...
									<div class="pro-body p-0">
										<div class="form-row pro-pad">
											...
											<div class="form-group col-md-6">
												...
												<input type="text" name="mobile" class="form-control" id="" placeholder="Phone" value="{{ Auth::user()->mobile }}"><br>
											...
											<div class="form-group col-md-6 {{ $errors->has('id_type') ? 'has-error' : '' }}">
												...
												<select class="form-control" name="id_type" id="id_type" value="{{ $profile->id_type ?? '' }}">
													...
													<option {{ $profile->id_type == 'PAN' ? 'selected' : ''}} value="PAN">{{ __('PAN Card') }}</option>
													<option {{ $profile->id_type == 'Aadhar' ? 'selected' : ''}} value="Aadhar">{{ __('Aadhar Card') }}</option>
													<option {{ $profile->id_type == 'Passport' ? 'selected' : ''}} value="Passport">{{ __('Passport') }}</option>
													<option {{ $profile->id_type == 'Voter' ? 'selected' : ''}} value="Voter">{{ __('Voter Card') }}</option>
												</select>
												@else
												<select class="form-control" name="id_type" id="id_type" value="{{ $profile->id_type ?? '' }}">
													<option value="" selected="selected">Select option</option>
											...
											<div class="form-group col-md-6">
												<div class="" id="showdiv">
													<div {{ $errors->has('ID No') ? 'has-error' : '' }}">
														<label for="">{{ __('ID No') }}</label>
														<input type="text" name="id_no" value="{{ $profile->id_no ?? '' }}" class="form-control" id="id_no" placeholder="ID Number">
														@if ($errors->has('id_no'))
														<span class="help-block">
															<strong>{{ $errors->first('ID No') }}</strong>
														</span>
														@endif
													</div>
												</div>	
