...
class PhotosController extends Controller
{
	...
	{    
		...
		// { 			
		    	$files = $request->file('photo');
				//foreach($files as $key => $image_file)
				/*foreach($request->photo as $key => $image_file)
				{	
				...
				/*foreach($request->photo as $image_file)
				{
					$x = 20; //this to be chenged when quality require good based on resolution
					$img_extension = $image_file->getClientOriginalExtension();
					$img_filename = time().'.'.$img_extension;	
				...
				{	
					...
					{		
						...
						{	
							foreach($request->photo as $key => $image_file)
							{
								$x = 20; //this to be changed when quality require good based on resolution
								$img_extension = $image_file->getClientOriginalExtension();
								$img_filename = time().'.'.$img_extension;	
								$img = Image::make($image_file);				
								$img->save(public_path('/photo/'.$img_filename),$x);	
	...
	public function deleteAll(Request $request)
    {
		...
		foreach ($photos as $photo) 
		{
			...
			$GenrePhoto = (public_path('/photo/').$currentPhoto);
					
			if(file_exists($GenrePhoto))
			{
				@unlink($GenrePhoto);					
			}
			$photo->delete();
	...
	//for dropzone working hided for testing code above code original
	/*public function picstore(Request $request)
    {				
		//2 working
	    $image = $request->file('file');
