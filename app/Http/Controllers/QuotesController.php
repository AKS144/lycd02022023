...
class QuotesController extends Controller
{
                ...
                if($quotes->save())
                {
                    ...
                    else 
                    {
                       ...
                       //dd($job);
                        ...
                        if($job->save())
                        {
                            Quotes::where('id', '=', $quotes->id)->update([
                                'job_id' => $job->id
                            ]);
                        }
                    }        
   ...
   /* public function artist_price(Request $request, $id)
    ...
    {      
        ...
        if($quote)
        {
            return redirect()->back()->with('error','You have already applied for this date');
        }
        else
        {      
            // Validation
            ...
            $ch = Quotes::whereId($id)->update(['client_status' => 1]);  
            ...
            if($ch)
            {
                $job = Quotes::where('job_id','=',$q->job_id)->where('id','!=',$id)->update([
                    'artist_accept' => 3 
                ]);
